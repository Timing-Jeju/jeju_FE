...
4. Green: HTTPS origin과 짧은 수명 access token을 요구하는 read-only staging harness를 추가했다. legal/place/profile/saved-place/trip/notification-preference와 restart 재조회, seeded trip의 detail/schedule을 mock 없이 실제 HTTP로 검사한다.
5. Refactor: runtime 37개 operation을 화면 domain에 한 번씩 매핑하고 호출/응답/error/restart-hydration 근거 파일이 실제로 존재하는지 테스트로 고정했다.
